"""Registry for managing singleton objects."""
...
class RegistrySingleton:
    ...
    >>> from deepdrivewe.workflows.registry import registry
    >>> from deepdrivewe.workflows.clear_torch_cuda_memory_callback

    >>> registry.register(
    ...     MyExpensiveTorchClass,
...
def _register_cls_decorator(cls: Callable[P, T]) -> Callable[P, T]:
    ...
    class SingletonWrapper(cls):  # type: ignore[valid-type,misc]
        def __new__(__cls, *args: P.args, **kwargs: P.kwargs) -> Any:  # noqa: N804
...
def register(
    ...
    Example
    -------
    Register a function once and then get the singleton instance in future
    calls:
